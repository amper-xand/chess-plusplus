...
namespace core::generators::knights {
    ...
    bitboard gen_knight_moves(square index) {
        ...
        // Top-most left
        if (row <= 5 && 1 <= col)
            moves |= bitboard::bitpos(index.up(2).right());

        // Bottom-most left
        if (2 <= row && 1 <= col)
            moves |= bitboard::bitpos(index.down(2).right());

        // Top-most right
        if (row <= 5 && col <= 6)
            moves |= bitboard::bitpos(index.up(2).left());

        // Bottom-most right
        if (2 <= row && col <= 6)
            moves |= bitboard::bitpos(index.down(2).left());

        // Top left-most
        if (row <= 6 && 2 <= col)
            moves |= bitboard::bitpos(index.up().right(2));

        // Bottom left-most
        if (1 <= row && 2 <= col)
            moves |= bitboard::bitpos(index.down().right());

        // Top right-most
        if (row <= 6 && col <= 5)
            moves |= bitboard::bitpos(index.up().left(2));

        // Bottom right-most
        if (1 <= row && col <= 5)
            moves |= bitboard::bitpos(index.down().left(2));
